...
const FlashSale = () => {
  ...
  const RenderItem = ({item = {}}) => {
    ...
    return (
      <View style={styles.itemWrap} horizontal={true}>
        <TouchableOpacity style={styles.wrapContent} onPress={_handlePress}>
            ...
            <View style={styles.wrapSold}>
              ...
              {qty_sold !== qty_flashsale ? (
                <Text style={styles.wrapSoldText}>{`${i18n.t(
                  'home_flash_sale:sold',
                )} ${qty_sold}`}</Text>
              ) : (
                <Text style={styles.wrapSoldText}>
                  {i18n.t('home_flash_sale:sold_out')}
                </Text>
              )}
            </View>
          </View>
